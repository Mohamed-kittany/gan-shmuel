...
class Deployment:
    ...
    def deploy_service(self, service: DockerService, environment: str) -> bool:
        ...
        try:
            ...
            # Run tests after deploying to the environment
            logger.info(f"New version of {service.service_name} deployed successfully to {environment} environment.")
            # return self.run_tests(service.service_dir)

        except Exception as e:
            logger.error(f"Deployment failed for {service.service_name} in {environment}: {e}")
    ...
    def check_ports_open(self, project_name: str, required_ports: List[int]) -> bool:
        ...
        for port_range in required_ports:
            for container_port in container_ports:
                if f"{port_range}" in container_port:
    ...
    def switch_traffic(self, old_project: str, new_project: str) -> None:
        ...
        if self.check_ports_open(new_project, [8080, 8090, 3000]):
            logger.info(f"New project {new_project} has the required ports open. Switching traffic.")
            # Stop the old project, because it’s no longer needed
            DockerService(self.repo_dir, old_project, '', False).stop(old_project)
        else:
            logger.error(f"{new_project} did not pass port checks. Keeping traffic on {old_project}.")
            DockerService(self.repo_dir, new_project, '', False).stop(new_project)
    ...
    def deploy_to_prod(self, service: DockerService) -> bool:
        ...
        # Check if the Green environment has required ports open
        if self.check_ports_open(green_project, [8080, 8090, 3000]):
            logger.info(f"Green environment for {service.service_name} is healthy. Switching traffic.")
            self.switch_traffic(blue_project, green_project)
            return True
        else:
            logger.error(f"Green environment for {service.service_name} is not healthy. Stopping Green deployment.")
            service.stop(green_project)
            return False
        ...
        try:
            ...
            with ThreadPoolExecutor() as executor:
                deploy_results = list(executor.map(
                    lambda s: self.deploy_service(DockerService(self.repo_dir / s[0], s[0], s[1], s[2]), 'test'), 
                    services
                ))
            ...
            if all(deploy_results):
                ...
                # Deploy both services to production (Blue-Green deployment)
                with ThreadPoolExecutor() as executor:
                    executor.map(
