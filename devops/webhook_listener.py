...
def github_webhook():
    try:
        ...
        commit_owner_email = commit_info.get('committer', {}).get('email', 'Unknown')
        commit_msg = commit_info['message']

        logger.info(f"Webhook received: Commit by {author} to branch {branch_name} at {commit_info['timestamp']}")
        logger.info(f"Commit email: {commit_owner_email}")
        ...
        success = main(rollback=False)

        # Check success and return appropriate status code
        if success:
            return jsonify({"status": "success", "message": "CI pipeline completed successfully"}), 200
        else:
            return jsonify({"status": "failure", "message": "CI pipeline failed"}), 500
